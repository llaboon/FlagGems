...
# Above this many output elements, use the split path (native interior copy +
...
#     strided copy of the input (handled by the native `_copy_from` engine),
#   * top/bottom strips -- (pad_top+pad_bottom) full output rows per (n, c),
#   * left/right edge columns of the interior rows (edge kernel + copies).
#
...
# The temp is then copied into the output's two edge views with the native
# `_copy_from` engine (a direct triton store into those strided column runs
# cannot be proven contiguous and degrades to ~2 ns/elem scatter).
@triton.jit
...
def launch_reflection_pad2d(input: torch.Tensor, padding, out: torch.Tensor = None):
    ...
    # Split path for large outputs: the interior is a pure strided copy that
    # the native `_copy_from` engine runs at 0.3-1.7 TB/s, while the flat gather
    # kernel is pinned at ~15 GB/s (discrete-gather wall) for EVERY element.
    # Only the (small) reflection border needs gather-style work here.
    # Below the threshold the single-launch flat kernel wins (the launch floor
    # dominates small shapes and the flat kernel already beats vendor there).
    if total_out >= _SPLIT_THRESHOLD:
        # 1) interior: native strided copy (gems overrides copy_/copy but never
        #    `_copy_from`, same recipe as constant_pad_nd).
        interior = out.narrow(-2, pad_top, H_in).narrow(-1, pad_left, W_in)
        torch.ops.aten._copy_from(x, interior, False)
        ...
        if pad_top + pad_bottom > 0:
            ...
            block_tb = min(8192, max(128, triton.next_power_of_2(max(len_top, len_bot))))
